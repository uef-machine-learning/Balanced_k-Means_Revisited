...
Coordinate Point::getCoord() {
	return coord;
}

void Point::assignToCluster(Cluster* clusters, int numClusters, int numIter, double penaltyNow, double* penaltyNext, 
	...
	if (numIter == 0) {
		...
		assignmentWithPenalty(clusters, numClusters, penaltyNow, penaltyNext, partlyRemainingFraction, oldClusterId);
	}
...
void Point::assignmentWithPenalty(Cluster* clusters, int numClusters, double penaltyNow, double* penaltyNext,
